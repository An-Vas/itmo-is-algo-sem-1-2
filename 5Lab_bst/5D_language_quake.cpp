...
Command* findNumOfCommand (string inputLabel, int totalCommandsNumber, Command* firstCommand){
    int inputLabelLenght = inputLabel.length();
    Command* curCommandToCheck = firstCommand;
    while (curCommandToCheck != 0){
        string strToCheck = curCommandToCheck->val;
        bool itIsThisLabel = true;
        if (strToCheck[0] != ':') {curCommandToCheck = curCommandToCheck->next; continue; }
        ...
        if (itIsThisLabel == true) {
            return curCommandToCheck;
            break;
        }
        curCommandToCheck = curCommandToCheck->next;
    }
...
int main ()
{
    ...
    Command* lastCommandPointer = firstCommand;
    ...
    while (fin>>curStr){
        ...
        newCommand->val = curStr;
        newCommand->next = 0;
        newCommand->prev = lastCommandPointer;

        if (firstCommand == 0) {
            firstCommand = newCommand;
        }
        else lastCommandPointer->next = newCommand;

        lastCommandPointer = newCommand;
        totalCommandsNumber++;
    }

    Command* curCommandPointer = firstCommand;


    while (curCommandPointer != 0) {

        string curCommand = curCommandPointer->val;
        ...
        if (curCommand[0] == '+'){
            ...
            curCommandPointer = findNumOfCommand(curCommand, totalCommandsNumber, firstCommand);
            ...
            if (x == 0) {
                ...
                curCommandPointer = findNumOfCommand(inputLabel_A, totalCommandsNumber, firstCommand);
            }
        ...
        curCommandPointer = curCommandPointer->next;
    }
